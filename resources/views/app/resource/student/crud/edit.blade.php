                                ...
                                <p class="text-xs text-white p-1">
                                    @foreach ($student->parents()->first()->guardian->particulars->phones as $phone)
                                        ...
                                        @if (!$loop->last)
                                            {{-- {{__(", ")}} --}}
                                        @else
                                            , <a title="Edit phone numbers" href="{{route('users.show',$student->parents()->first()->guardian->particulars->id)}}" class="hover:bg-yellow-400 text-white bg-yellow-300 rounded px-2">Edit</a>
                                        @endif
                                    @endforeach
                                </p>
                            </div>
...
@section('title')
<p class="text-lg text-gray-600 font-thin">Edit <span class="font-semibold">{{ucwords($student->particulars->first_name.' '.$student->particulars->sur_name)}}</span> info</p>
@endsection
