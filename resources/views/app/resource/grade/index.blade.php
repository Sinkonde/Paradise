    ...
    <div class="w-100 flex justify-center ">
        <div class="w-full flex flex-col py-4 bg-white shadow">
            <div class="w-full flex justify-between items-center px-4 pb-4 mb-4 border-b" x-data="{'show':false}">
                <p class="text-xl md:text-xl text-gray-600 font-thin"><span @click="show=!show" class="fi fi-angle-down hover:text-gray-600 text-gray-300 cursor-pointer text-sm"></span> <b class="cursor-pointer" @click="show=!show">{{$level_name?$level_name:__('All')}}</b> Grades ({{$grades->count()}} grades)</span></p>
                <div x-show="show" class="absolute bg-white rounded mt-28 flex flex-col border shadow-lg" @click.away="show=false">
                    <a class="px-2 text-sm py-2 hover:bg-gray-100 text-xs" href="{{route('grades.index')}}">All</a>
                    @foreach ($levels as $level)
                        <a class="px-2 text-sm py-1 hover:bg-gray-100 text-xs" href="{{route('grades.index',['for_level' => $level->id])}}">{{$level->name}} grades</a>
                     @endforeach
                </div>

                <a class="absolute right-5 bottom-5" href="{{route('grades.create')}}">
                    ...
                    <tbody>
                        @foreach ($grades as $grade)
                            <tr class="hover:bg-gray-100 text-xs">
                                ...
                                <td class="py-2">{{$grade->name}}</td>
                                <td class="py-2">{{$grade->grade}}</td>
                                <td class="py-2">{{$grade->level->name}}</td>
                                <td class="py-2 flex flex-row">
                                    <a class="mr-4 text-blue-400 hover:underline hover:text-blue-600" href="{{route('grades.edit', $grade->id)}}">Edit</a>
